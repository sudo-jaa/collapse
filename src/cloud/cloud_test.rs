...
mod tests {
    ...
    use crate::{solar_mass, Coordinates};
    use float_cmp::assert_approx_eq;
    use std::time::Instant;
    use uom::fmt::DisplayStyle::Abbreviation;
    use uom::si::amount_of_substance::mole;
    use uom::si::f64::{
        AmountOfSubstance, Length, Mass, MassDensity, MolarMass, Pressure,
    ...
    fn cloud_volume_test() {
        ...
        let cloud = MolecularCloud::new(
            coords,
            l1,
            UniformGas::generate_massdensity(
            ...
            vec![
                ...
                (
                    Molecule::new(vec![(Element::Carbon(CarbonIsotope::Carbon), 1)]),
                    1.5,
                ),
    ...
    fn molecular_cloud_pressure() {
        ...
        let substance =
            moles::from_pressure_volume_temperature(Pressure::new::<pascal>(1.0), vol, temp);
        assert_eq!(
            substance,
            AmountOfSubstance::new::<mole>(0.017181765006103723)
        );

        println!(
            "{:?}",
            moles::from_pressure_volume_temperature(
                MolecularCloud::pressure(),
                volume::sphere_volume_from_length(Length::new::<light_year>(25.0)),
                temp
            )
        );
